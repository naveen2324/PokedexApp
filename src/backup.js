...
function Pokemondetails() {
  ...
  useEffect(() => {
    axios.get("https://pokeapi.co/api/v2/pokemon/" + id).then(async (response) => {
      ...
      for (let i = 0; i < moves.length; i++) {
        for (let j = moves[i].version_group_details.length - 1; j > -1; j--) {
          if (moves[i].version_group_details[j].move_learn_method.name == "level-up" && moves[i].version_group_details[j].version_group.name == latest_version) {
            axios.get(moves[i].move.url).then((response2) => {
              ...
              var type = response2.data.type.name[0].toUpperCase() + response2.data.type.name.slice(1,);
              var url = "#/moveDetails/" + response2.data.id;
      ...
      console.log(LevelUpMoveDetails);
            ...
            axios.get(moves[i].move.url).then((response2) => {
              ...
              <a href={url}>
                <div id={styles.move_row}>
                  <p class={styles.move_name}>{name}</p>
                  <p class={styles.move_type} style={{ background: type_colors[type.toLowerCase()] }}>{type}</p>
                  <p class={styles.move_power}>{power}</p>
                  <p class={styles.move_accuracy}>{accuracy}</p>
                </div>
              </a>
              ]);
